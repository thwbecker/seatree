...
#		if os.path.isfile(matrix_out) and (self.data_type == self.old_data_type) and \
#					(self.res == self.old_res) and (self.nlay == self.old_nlay) and \
#					(self.ray_type == self.old_ray_type):
			if self.verbose > 0:
				print matrix_out
...
#			self.scriptRunner.runScript(command)
	...
	def makeSolution(self):
		...
		# if solution file does not exist - compute solution
		else :
...
#		if len(nlaystr) < 2:
#			nlaystr = "0"+nlaystr
#		gmtfile = self.tempDir + "layer_" + nlaystr
...
#		if self.myPlotter.adjust:
		for line in open(gmtfile):
			...
			if val < min: min = val
			if val > max: max = val
		print "layer_",nlaystr," min= ", min, "max =", max
		tr = self.myPlotter.grdNiceCmpRange(min, max,cutoff = 0.9)
		self.myPlotter.setColorbarInterval(tr[3])
		self.myPlotter.makeCPT(tr[0],tr[1],tr[2], self.colormap, setBackgroundMax=True)
...
#			shutil.copy2(self.colormap, self.tempDir + mytomo)
#			print "colormap copied to working directory"
...
#			print ret
			...
			if not os.path.exists(self.tempDir + "vprofile.txt"):
				...
				if os.path.exists(nlaypath): print 'and copied to ' + self.layers
				else: print 'but not successfully copied to ' + self.layers
		else: print 'Layers were previously extracted. Using old files: ',self.layer_files + "##.dat" 
	
	def calcMidLayerDepth(self):
		#
		# Calc mid-layer depths
		#
#		vprofile = self.tempDir + "vprofile.txt"
...
#					self.layer_depth[i] = avg
		...
		i = 0
		for line in self.layerDepth:
			self.layerDepth[i] = line.strip()
			i += 1
#		print self.layerDepth
	...
	def plot(self):
		...
		if self.myPlotter.adjust :
			self.adjustGMTInput()
		#Set PostScript File
#		self.myPlotter.initPSFile(fileName,self.colormap)
		self.myPlotter.initPSFile(fileName)
		# set colormap
...
#		self.myPlotter.plotAnomalyBoxes(fileName)
		if(self.myPlotter.drawPlateBounds):
			self.myPlotter.drawPlateBoundaries()
...
#			layerIndex = self.nlayplot - 1
		...
		#Close PS File
#		self.myPlotter.closeLarry3dPSFile(self.colormap)
		self.myPlotter.closePSFile()
...
#			sys.exit("cannot continue: solution was not parsed")
		else:
...
#			print "norm = ", self.norm
	...
	def getRoughness(self):
		...
		if os.path.exists(roughness):
			...
			self.rdampf =	 line[1];
		else: print "Error: problem with vox2roughness - roughness.txt was not generated" 
...
#				return
		...
		if killed:
			...
			print "calculation finished in " + str(step*pollInterval) + " seconds" # str(end - start) + " seconds"
