...
class ConManInstaller:
	...
	def getConManPath(self):
		...
		if path == None:
			print("ConMan could not be located. You can either check it out from SVN, or supply the"\
						+ " path manually.")
			response = input("Would you like to check it out from SVN?  (y/n default: y)")
			if not installUtils.isResponseNo(response):
				...
				while not self.isConManPathValid(path):
					...
					path = input("Enter the path to ConMan: ")
	...
	def checkUseConManPaths(self, paths):
		for path in paths:
			if self.isConManPathValid(path):
				...
				response = input("Would you like to use this installation of ConMan? (y/n default: y)? ")
	...
	def checkBuildConMan(self):
		...
		response = input("Build ConMan Binaries (note: you must do this once) (y/n default: y)? ")
	...
	def setupMakefiles(self, path):
		...
		if not (f77):
			...
			newF77 = input("Would you like to use gfortran or ifort (must be in system path) (default: gfortran): ")
			if (not newF77):
				newF77 = "gfortran"
			f77 = newF77
