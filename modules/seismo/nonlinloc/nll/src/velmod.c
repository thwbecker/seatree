...
int id_num;
{
    ...
    if (addr == NULL) {
		puterr("ERROR: adding vertex, no memory?");
		return(addr);
    }
...
int id_num;
{
    ...
    if (addr == NULL) {
		puterr("ERROR: adding edge, no memory?");
		return(addr);
    }
...
int id_num;
{
    ...
    if (addr == NULL) {
		puterr("ERROR: adding polygon, no memory?");
		return(addr);
    }
...
int id_num;
{
    ...
    if (addr == NULL) {
		puterr("ERROR: adding solid, no memory?");
		return(addr);
    }
...
int set_poly_normals(struct polygon *addr)
{
	...
	struct vertex *vaddr1, *vaddr2, *vaddr_test;
   	...
   	if ((addr->norm = (struct normals *)
		...
		puterr("ERROR: allocating normals memory.");
