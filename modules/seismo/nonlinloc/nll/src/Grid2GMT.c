...
int main(int argc, char *argv[])
{
	...
	if (argc < 6 || cplotmode == '\0' || (cplotmode == 'V' && argc != 10)
		|| (cplotmode == 'H' && argc != 7)
		|| ((cplotmode == 'L') && !(argc == 6 || argc == 9)))
	{
		puterr("ERROR wrong number of command line arguments.");
		disp_usage(PNAME,
"<controlfile> <gridroot> <outroot> V [G][S][Ennndx][M] ix1 iy1 ix2 iy2");
		disp_usage(PNAME,
"<controlfile> <gridroot> <outroot> VL [G][S][Ennndx][M] lat1 long1 lat2 long2");
		disp_usage(PNAME,
	...
	strcpy(fninput, argv[2]);
	strcpy(fnoutput, argv[3]);
	// parse plottype
	num_arg_elements = parseCommandArgument(argv[5], &cdatatype, arg_elements);

	if (cplotmode == 'V') {
		if (clatlongmode == 'L') {	// lat/long specification of ends
			istat = 0;
			istat += sscanf(argv[6], "%lf", &vlat1);
			istat += sscanf(argv[7], "%lf", &vlong1);
			istat += sscanf(argv[8], "%lf", &vlat2);
			istat += sscanf(argv[9], "%lf", &vlong2);
		} else {	// grid index specification of ends
			istat = 0;
			istat += sscanf(argv[6], "%d", &ix1);
			istat += sscanf(argv[7], "%d", &iy1);
			istat += sscanf(argv[8], "%d", &ix2);
			istat += sscanf(argv[9], "%d", &iy2);
		}
		sprintf(args_str, "%s_%s_->_%s_%s", argv[6], argv[7], argv[8], argv[9]);
	} else if (cplotmode == 'H') {
		sscanf(argv[6], "%d", &izlevel);
...
//grid0.iSwapBytes = 1;
		...
		if (clatlongmode == 'L') {
			...
			if (ix1 < 0 || ix1 >= grid0.numx || iy1 < 0 || iy1 >= grid0.numy ||
				...
				puterr("ERROR lat long points not inside grid.");
		...
		int ix1, int iy1, int ix2, int iy2, int izlevel, GridDesc *pgrid0)
{
	...
	{
		puterr("ERROR opening gmt output file.");
		return(-1);
	}
	...
	fprintf(fp_gmt,
"psbasemap -R0.5/1.0/0.5/1.0 -Bf10 -JX%lfc/%lfc -K > %s.ps\n\n",
	...
	//} else {
		...
		{
			if (message_flag >= 1)
				putmsg2("INFO: cannot open hypocenter file", fn_hypo);
		}
		...
		if (fp_hypo != NULL && ((ihypo = GetHypLoc(fp_hypo, NULL, &Hypo, Arrival,
			...
			fprintf(fp_gmt,
"pstext -R0.5/1.0/0.5/1.0 -Bf10 -JX%lfc/%lfc -K -O << END >> %s.ps\n%lf %lf %d %d %d %d %s\nEND\n\n",
			PLOT_WIDTH, PLOT_HEIGHT, fn_ps_output,
...
printf("arg_elements[2] <%s>  RES MIN NUM RADINGS: %d\n", arg_elements[2], res_min_num_readings);
	...
	if (cplotmode == 'L') {
		...
		xshift = 1.75;
		yshift = 3.75;
		sprintf(shift_str, "-X%lfc -Y%lfc", xshift, yshift);
//if (cdatatype != 'R')
		...
		sprintf(shift_str, "-X%lfc -Y%lfc", xshift, 0.0);
//if (cdatatype != 'R')
		GenGridViewGMT(pgrid0, 'L', cdatatype, arg_elements, num_arg_elements,
			Hypo.ix, 0, Hypo.ix,
			pgrid0->numy, 0,
	...
	if (cdatatype != 'S' && cdatatype != 'E' && cdatatype != 'M' && cdatatype != 'R') {
		...
		if (pgrid0->type == GRID_PROB_DENSITY) {
			...
			strcpy(cpt_str, "");
			strcpy(scale_label_str, "Depth (km)");
		...
		fprintf(fp_gmt,
"psscale -C%s.%scpt -D%lfc/%lfc/14.0c/0.5ch -B:\"%s\": -X%lfc -Y%lfc $SCALE_FLAG -K -O >> %s.ps\n\n",
...
"pstext $JVAL $RVAL $BVAL -N -K -O << END >> %s.ps\n%lf %lf %d %d %d %d %s\nEND\n\n",
	...
	fprintf(fp_gmt,
"pstext -R0.5/1.0/0.5/1.0 -Bf10 -JX%lfc/%lfc -X%lfc -Y%lfc -O << END >> %s.ps\n%lf %lf %d %d %d %d %s\nEND\n\n",
		PLOT_WIDTH, PLOT_HEIGHT, -xshift_cum, -yshift_cum,
		fn_ps_output,
		0.501, 0.501, ANNOTATION_FONT_SIZE, 0, ANNOTATION_FONT,
		1, signature_str);

	fprintf(fp_gmt, "echo \"Finished!  Output is: %s.ps\"\n", fn_ps_output);
...
*/
	...
	if (cdatatype == 'G') {
		...
		{
			puterr("ERROR opening gmt grid output file.");
			return(-1);
		}
	}
	...
	if (cviewmode == 'H')
	{
		...
		for (ny = pgrid->numy - 1; ny >= 0; ny--)
		{
			...
			{
				value = ReadGrid3dValue(fp_grid, nx, ny,
					izlevel, pgrid);
/*
if (nx % 20 == 0 && ny % 20 == 0) {
byteval.fval = value;
...
*/
		...
		vert_dgrid = pgrid->dy;
		strcpy(horiz_label, "X(km)");
		strcpy(vert_label, "Y(km)");
	...
	else if (cviewmode == 'L' && ix1 == ix2)
	{
		...
		for (ny = iy2 - 1; ny >= iy1; ny--)
		{
			...
			{
				value = ReadGrid3dValue(fp_grid, ix1, ny, nz,
					pgrid);
				if (iAzAngle || iDipAngle)
					value = CalcAngleValue(value,
						iAzAngle, iDipAngle);
		...
		strcpy(horiz_label, "Z(km)");
		strcpy(vert_label, "Y(km)");
		strcpy(horiz_label_deg, "Z(km)");
		strcpy(vert_label_deg, "Lat(deg)");
		yscalefact = 1.0;
		zscalefact = 1.0;
...
/*		for (nz = 0; nz < pgrid->numz; nz++)*/
		...
		vert_dgrid = pgrid->dz;
		strcpy(horiz_label, "Y(km)");
		strcpy(vert_label, "Z(km)");
		strcpy(horiz_label_deg, "Lat(deg)");
		strcpy(vert_label_deg, "Z(km)");
		yscalefact = -1.0;
		zscalefact = 1.0;
		rect2latlon(proj_index_output, pgrid->origx + (double) ix1 * pgrid->dx,
	...
	else if (iy1 == iy2)
	{
		...
		for (nz = pgrid->numz - 1; nz >= 0; nz--)
		{
			...
			{
				value = ReadGrid3dValue(fp_grid, nx, iy1, nz,
					pgrid);
				if (iAzAngle || iDipAngle)
					value = CalcAngleValue(value,
						iAzAngle, iDipAngle);
	...
	else if (cviewmode == 'V')
	{
		...
		if (pgrid->dx != pgrid->dy) {
			puterr("ERROR cannot have datatype = S when grid dx != dy.");
			exit(-1);
		}
		...
		for (nz = pgrid->numz - 1; nz >= 0; nz--)
		{
			for (nhypot = 0; nhypot < hypotenouse; nhypot++)
			{
				...
				value = ReadGrid3dValue(fp_grid, nx, ny, nz,
					pgrid);
				if (iAzAngle || iDipAngle)
					value = CalcAngleValue(value,
						iAzAngle, iDipAngle);
		...
		puterr("ERROR cannot have datatype = S with oblique Vertical section.");
		exit(-1);
	    } else if (cdatatype == 'E') {
		puterr("ERROR cannot have datatype = E with oblique Vertical section.");
		exit(-1);
	    } else if (cdatatype == 'M') {
		puterr("ERROR cannot have datatype = M with oblique Vertical section.");
		exit(-1);
	    } else if (cdatatype == 'R') {
		puterr("ERROR cannot have datatype = R with oblique Vertical section.");
		exit(-1);
	    }
...
*/
	...
	if ((fp_stations = fopen(fn_stations, "r")) != NULL)
	{
		...
		if (message_flag >= 1)
			putmsg2("INFO: cannot open station list file", fn_stations);
...
printf("GetContourInterval vtick_int R: ");
	...
	*pylen = (vert_max - vert_min) * plot_scale;
	...
	fprintf(fp_gmt, "# Rect x/y in km\n");
	fprintf(fp_gmt, "set JVAL = \'-Jx%lfc/%lfc -Jz%lfc\'\n", plot_scale,
...
printf("GetContourInterval vtick_int G: ");
		...
		fprintf(fp_gmt, "# Latitude/Longitude in degrees\n");
		sprintf(gmt_JVAL, "set JVAL_LL = \'%s -Jz%lfc\'",
...
"grdimage %sgmt -C%s.conf.cpt $JVAL $RVAL $BVAL -K -O >> %s.ps\n\n",
		...
		fprintf(fp_gmt,
"grdcontour %sgmt -A- -C%s.conf.cpt $JVAL $RVAL $BVAL -K -O >> %s.ps\n\n",
		fn_gmtgrd, fn_root_output, fn_ps_output);
...
"grdimage %sgmt -C%s.misfit.cpt $JVAL $RVAL $BVAL -K -O >> %s.ps\n\n",
		...
		fprintf(fp_gmt,
"grdcontour %sgmt -A- -C%s.misfit.cpt $JVAL $RVAL $BVAL -K -O >> %s.ps\n\n",
		fn_gmtgrd, fn_root_output, fn_ps_output);
...
int MakeConfCPT(char* fn_in, char* fileout)
{
	...
	{
		puterr2("ERROR opening confidence level file", fn_in);
		return(-1);
	}
	sprintf(fn_out, "%s.conf.cpt", fileout);
	if ((fp_out = fopen(fn_out, "w")) == NULL)
	{
		puterr2("ERROR opening confidence level cpt file", fn_out);
		return(-1);
	}
...
int MakeMisfitCPT(char* fileout, double value_max, double contour_interval)
{
	...
	if ((fp_out = fopen(fn_out, "w")) == NULL)
	{
		puterr("ERROR opening misfit cpt file.");
		return(-1);
	}
...
int MakeTopoCPT(char* fileout)
{
	...
	{
		puterr("ERROR opening topo cpt file.");
		return(-1);
	}
		...
		FILE* fp_gmt, char* fn_ps_output, int doLatLong)
{
	...
	if ((fp_gmtxy = fopen(fn_gmtxy, "r")) == NULL
		...
		if ((fp_gmtxy = fopen(fn_gmtxy, "w")) == NULL)
			puterr2("ERROR: cannot open map xy output file", fn_gmtxy);
		if ((fp_gmtlatlon = fopen(fn_gmtlatlon, "w")) == NULL)
			puterr2("ERROR: cannot open map lat/lon output file", fn_gmtxy);
		...
		if ((fp_map = fopen(mapfile[nmapfile].name, "r")) == NULL) {
			puterr2("ERROR: cannot open map file",
				mapfile[nmapfile].name);
			return(-1);
		}
	...
	FILE* fp_io, char *plt_code_in, char *GMTcolor)
{
	...
	if (plt_code[0] == '1') {
		...
		fprintf(fp_io,
"# Maximum Likelihood\npsxy $JVAL $RVAL %s -Sa%lf -K -O << END >> %s.ps\n",
	...
	if (plt_code[1] == '1') {
		...
		fprintf(fp_io,
"# Expectation\npsxy $JVAL $RVAL %s -G0 -Sc%lfc -K -O << END >> %s.ps\n",
...
npts_ellipse = 20;
		ellipse_array =
			...
			fprintf(fp_io,
"# Error Ellipsoid\npsxy $JVAL $RVAL %s -M -K -O << END >> %s.ps\n>\n",
...
//	if (plt_code[0] == '1') {
		...
		if (view_type == 'Y') {
			...
			sprintf(J_string, "-Jx%lfc/%lfc", plot_scale, plot_scale);
			fprintf(fp_io,
"# Focal Mechanism\npscoupe %s %s %s %s %s -Sa%lfc -N -K -O << END >> %s.ps\n",
			...
			fprintf(fp_io,
"# Focal Mechanism\npsmeca $JVAL $RVAL %s %s -Sa%lfc -K -O << END >> %s.ps\n",
		...
		SourceDesc* staList, int nstations, double scale, int min_num_readings)
{
	...
	if ((fp_in = fopen(fn_nlloc_stat, "r")) == NULL) {
		puterr2("Cannot open phase statistics file", fn_nlloc_stat);
		return(-1);
    	}
