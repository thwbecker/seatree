...
int main(int argc, char *argv[])
{
	...
	if ((fp_gmt_output = fopen(fn_gmt_output, "w")) == NULL) {
		puterr("ERROR: opening eq times GMT output file.");
		exit(EXIT_ERROR_FILEIO);
	}

	sprintf(fn_gmt_output, "%s_az.gmtxy", fn_eq_output);
	if ((fp_gmt_output_az = fopen(fn_gmt_output, "w")) == NULL) {
		puterr("ERROR: opening eq times GMT azimuth output file.");
		exit(EXIT_ERROR_FILEIO);
	}
...
"Calculating travel time for Source: %s  X %.2lf  Y %.2lf  Z %.2lf  OT %.2lf",
		...
		if (time2eq_mode == MODE_STA_TO_SRCE)
			putmsg(3, MsgStr_srce);
		else
			putmsg(2, MsgStr_srce);
			...
			if (imech == MECH_DOUBLE &&
				...
				if ((ipolarity =  CalcFirstMotion(filename_angle,
					&grid0, Event, Station + nsta))
						== 0)
				/*puterr("ERROR: calculating polarity.")*/;
			}
		...
		SourceDesc*  pevent, StationDesc* psta, int num_sta)
{
	...
	if (WriteArrival(fpout, parr, IO_ARRIVAL_OBS) < 0) {
		puterr("ERROR: writing arrival time to disk.\n");
		return(-1);
	}
...
int ReadTime2EQ_Input(FILE* fp_input)
{
	...
	if (!flag_trans)
		puterr("INFO: no transformation (TRANS) params read.");
	if (!flag_mode) {
	...
	if (!flag_outfile)
		puterr("ERROR: no i/o file (EQFILES) params read.");
	if (!flag_event && !flag_source)
		puterr(
"ERROR: no event (EQEVENT) or source (EQSRCE) params read.");
	if (!flag_stations)
		puterr("ERROR: no station (EQSTA) params read.");
	if (!flag_qual2err)
		puterr("ERROR: no quality to error (EQQUAL2ERR) params read.");
	if (!flag_mech)
		puterr("INFO: no mechanism (EQMECH) params read.");
	if (!flag_vp_vs)
		puterr("INFO: no Vp/Vs (EQVPVS) params read.");
...
int GetTime2EQ_Mode(char* line1)
{
	...
	if (strcmp(str_mode, "SRCE_TO_STA") == 0)
		...
		puterr2("ERROR: unrecognized mode:", str_mode);
		return(-1);
	}
...
int GetTime2EQ_Stations(char* in_line)
{
	...
	if (NumStations >= MAX_NUM_STATIONS) {
		puterr("ERROR: to many stations, ignoring station.");
		return(0);
	}
...
int get_mech(char* line1)
{
	...
	if (strncmp(mech_type, "double", 6) == 0 ||
		...
		imech = MECH_NONE;
		strcpy(mstr, "None");
		puterr2("ERROR: unrecognized mechanism:", mech_type);
		return(-1);
	}
